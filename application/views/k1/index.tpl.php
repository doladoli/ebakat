				  ...
				  <div class="tab-pane <?php ++$i; if ($i==1) {?>active<?php }?>" id="tab_<?=$k->KR_ID?>">
                    ...
                    <div class="box-body table-responsive no-padding">
						<table class="table table-hover"><?php
							if($k->KR_KETERANGAN=='Beban Pengajaran')
							{ ?>
							<tr><th align="center">ITEM</th><?php
							
								for($i=1; $i<=5; $i++)
								{?>
									<th align="center">KOD KURSUS <?php echo $i;?></th><?php
								}?>
							</tr><?php	
							}?>
						...
						<?php
							...
							__jana($k->KR_ID, $this->pm, $this->corek1, 1, $k->KR_KETERANGAN);
						?>
...
function __jana($id, $conn, $lib, $l, $ket)
{ 
	$sub = $conn->get_sub_kriteria($id);
	//echo $id."test".$ket;
	if ($sub->num_rows() > 0) {
		...
		{			
			...
			echo $li;
			if ($x->num_rows() > 0) {
				__jana($s->KR_ID, $conn, $lib, $l+1, $ket);
			}
		}
		//echo "</table>";
	}
